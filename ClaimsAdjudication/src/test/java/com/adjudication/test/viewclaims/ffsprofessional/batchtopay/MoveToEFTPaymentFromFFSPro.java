package com.adjudication.test.viewclaims.ffsprofessional.batchtopay;
...
public class MoveToEFTPaymentFromFFSPro extends TestBase {

	MyLogger log = new MyLogger(MoveToEFTPaymentFromFFSPro.class.getName());
	...
	public void commonSteps() throws Exception {
		...
		batchId=idProp.getProperty("MoveToEFTPaymentFromFFSPro");
	}
	...
	public void moveToEFTPaymentFromBatchToPay() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("FFSPRO_EFTPAYMENT_TABLE_ID"),2,batchIdFromAPP);
	...
	public void moveToPaidFromEFTPaymentFFSPro() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("FFSPRO_EFTPAYMENT_TABLE_ID"),2,batchId);
			...
			softAssert.assertTrue(viewClaimsPage.checkApproveSuccessMsg(popUpMsg));
			ffsPro.clickOnPaidTab();
			commonActions.filterData(elementsProp.getProperty("FFSPRO_PAID_TABLE_ID"),1,batchId);
			softAssert.assertEquals( QMPage.getBatchIdFromPaidWithFilter(elementsProp.getProperty("FFSPRO_PAID_TABLE_ID")),batchId);
