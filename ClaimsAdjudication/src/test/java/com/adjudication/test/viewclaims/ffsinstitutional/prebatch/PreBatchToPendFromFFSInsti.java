...
public class PreBatchToPendFromFFSInsti extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPendFromFFSInsti.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPendFromFFSInsti");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			ffsInsti.clickOnPendTab();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("FFSINSTI_PEND_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("FFSINSTI_PEND_TABLE_ID")));
