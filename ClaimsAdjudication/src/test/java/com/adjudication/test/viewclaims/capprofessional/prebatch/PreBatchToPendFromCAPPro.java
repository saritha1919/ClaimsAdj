...
public class PreBatchToPendFromCAPPro extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPendFromCAPPro.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPendFromCAPPro");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			capPro.clickOnPendTab();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("CAPPRO_PEND_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("CAPPRO_PEND_TABLE_ID")));
