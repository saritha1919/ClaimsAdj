...
public class PreBatchToPendFromCAPInsti extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPendFromCAPInsti.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPendFromCAPInsti");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			capInsti.clickOnPendTab();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("CAPINSTI_PEND_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("CAPINSTI_PEND_TABLE_ID")));
