...
public class PreBatchToPayerReviewFromCAPPro extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPayerReviewFromCAPPro.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPayerReviewFromCAPPro");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			capPro.clickOnOnPayerReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("CAPPRO_PAYER_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("CAPPRO_PAYER_REVIEW_TABLE_ID")));
