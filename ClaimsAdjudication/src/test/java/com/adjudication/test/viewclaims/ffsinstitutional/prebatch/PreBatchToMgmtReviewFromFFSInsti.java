...
public class PreBatchToMgmtReviewFromFFSInsti extends TestBase{

	MyLogger log = new MyLogger(PreBatchToMgmtReviewFromFFSInsti.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToMgmtReviewFromFFSInsti");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			ffsInsti.clickOnOnManagementReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("FFSINSTI_MANAGEMENT_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("FFSINSTI_MANAGEMENT_REVIEW_TABLE_ID")));
