package com.adjudication.test.viewclaims.dental.prebatch;
...
public class PreBatchToMgmtReviewFromDental extends TestBase{

	MyLogger log = new MyLogger(PreBatchToMgmtReviewFromDental.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToMgmtReviewFromDental");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void commonSteps() throws Exception {
        ...
        QMPage.clickOnViewClaims(elementsProp.getProperty("DENTAL_PREBATCH_TABLE_ID"));
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			viewClaimsPage.clickOnManagementReviewButton();
			QMPage.selectReasonToPayerOrMgmtReview();
			QMPage.clickReviewInReasonToPayerOrMgmtReview();
			popUpMsg = messageProp.getProperty("MGMT_REVIEW_SUCCESS_MESSAGE_PRO");
			softAssert.assertTrue(viewClaimsPage.checkApproveSuccessMsg(genericMethods.insertString(popUpMsg, id, 70)));
			wait.waitForSeconds(2);
			dental.clickOnOnManagementReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("DENTAL_MANAGEMENT_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("DENTAL_MANAGEMENT_REVIEW_TABLE_ID")));
