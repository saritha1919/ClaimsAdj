...
public class PreBatchToPayFromDental extends TestBase {

	MyLogger log = new MyLogger(PreBatchToPayFromDental.class.getName());
	...
	public void commonSteps() throws Exception {
		...
		QMPage.selectClaimFromPrebatch(elementsProp.getProperty("DENTAL_PREBATCH_TABLE_ID"));
		dental.clickOnBtachClaimToPayButton();
	...
	public void movePrebatchToPay() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("DENTAL_BATCHTOPAY_TABLE_ID"),2,batchId);
			...
			softAssert.assertEquals(id,
					QMPage.getClaimNumber(elementsProp.getProperty("DENTAL_BATCHTOPAY_VIEWCLAIM_TABLE_ID")));
