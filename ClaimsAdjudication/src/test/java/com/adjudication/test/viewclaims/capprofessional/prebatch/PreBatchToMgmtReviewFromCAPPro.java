...
public class PreBatchToMgmtReviewFromCAPPro extends TestBase{

	MyLogger log = new MyLogger(PreBatchToMgmtReviewFromCAPPro.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToMgmtReviewFromCAPPro");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			capPro.clickOnOnManagementReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("CAPPRO_MANAGEMENT_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("CAPPRO_MANAGEMENT_REVIEW_TABLE_ID")));
