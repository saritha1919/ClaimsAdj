...
public class PreBatchToMgmtReviewFromFFSPro extends TestBase{

	MyLogger log = new MyLogger(PreBatchToMgmtReviewFromFFSPro.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToMgntReviewFromFFSPro");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void commonSteps() throws Exception {
        ...
        QMPage.clickOnViewClaims(elementsProp.getProperty("FFSPRO_PREBATCH_TABLE_ID"));
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			ffsPro.clickOnOnManagementReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("FFSPRO_MANAGEMENT_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("FFSPRO_MANAGEMENT_REVIEW_TABLE_ID")));
