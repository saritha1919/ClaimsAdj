package com.adjudication.test.viewclaims.capprofessional.deny;
...
public class DenyToPrebatchFromCAPProTest extends TestBase {

	MyLogger log = new MyLogger(DenyToPrebatchFromCAPProTest.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("DenyToPrebatchFromCAPProTest");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void commonSteps() throws Exception {
		try {
			...
			QMPage.clickOnViewClaims(elementsProp.getProperty("CAPPRO_DENIED_TABLE_ID"));
	...
	public void tryToMovePrebatchPayFromDenyFromCAPPro() throws Exception {
		try {
			commonSteps();
	...
	public void tryToMovePrebatchDenyFromDenyFromCAPPro() throws Exception {
		try {
			commonSteps();
			viewClaimsPage.clickOnPrebatchButton();
