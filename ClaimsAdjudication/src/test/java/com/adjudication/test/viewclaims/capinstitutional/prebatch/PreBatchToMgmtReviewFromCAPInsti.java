...
public class PreBatchToMgmtReviewFromCAPInsti extends TestBase{

	MyLogger log = new MyLogger(PreBatchToMgmtReviewFromCAPInsti.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToMgmtReviewFromCAPInsti");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			popUpMsg = messageProp.getProperty("MGMT_REVIEW_SUCCESS_MESSAGE");
			softAssert.assertTrue(viewClaimsPage.checkApproveSuccessMsg(genericMethods.insertString(popUpMsg, id, 70)));
			wait.waitForSeconds(2);
			capInsti.clickOnOnManagementReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("CAPINSTI_MANAGEMENT_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("CAPINSTI_MANAGEMENT_REVIEW_TABLE_ID")));
