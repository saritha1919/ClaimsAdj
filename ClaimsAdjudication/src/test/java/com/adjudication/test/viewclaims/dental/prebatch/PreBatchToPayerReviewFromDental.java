...
public class PreBatchToPayerReviewFromDental extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPayerReviewFromDental.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPayerReviewFromDental");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			viewClaimsPage.clickOnPayerReviewButton();
			QMPage.selectReasonToPayerOrMgmtReview();
			QMPage.clickReviewInReasonToPayerOrMgmtReview();
			popUpMsg = messageProp.getProperty("PAYERREVIEW_SUCCESS_MESSAGE_PRO");
			softAssert.assertTrue(viewClaimsPage.checkApproveSuccessMsg(genericMethods.insertString(popUpMsg, id, 70)));
			wait.waitForSeconds(2);
			dental.clickOnOnPayerReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("DENTAL_PAYER_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("DENTAL_PAYER_REVIEW_TABLE_ID")));
