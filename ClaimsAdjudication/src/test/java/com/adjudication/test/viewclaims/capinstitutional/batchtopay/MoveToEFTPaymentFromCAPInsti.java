...
public class MoveToEFTPaymentFromCAPInsti extends TestBase {

	MyLogger log = new MyLogger(MoveToEFTPaymentFromCAPInsti.class.getName());
	...
	public void commonSteps() throws Exception {
		...
		batchId=idProp.getProperty("MoveToEFTPaymentFromCAPInsti");
	}
	...
	public void moveToEFTPaymentFromBatchToPay() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("CAPINSTI_EFTPAYMENT_TABLE_ID"),2,batchIdFromApp);
	...
	public void moveToPaidFromEFTPaymentCAPInsti() throws Exception {
		try {
			...
			softAssert.assertEquals( QMPage.getBatchIdFromPaidWithFilter(elementsProp.getProperty("CAPINSTI_PAID_TABLE_ID")),batchId);
