...
public class PreBatchToPayerReviewFromFFSInsti extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPayerReviewFromFFSInsti.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPayerReviewFromFFSInsti");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			wait.waitForSeconds(2);
			ffsInsti.clickOnOnPayerReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("FFSINSTI_PAYER_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("FFSINSTI_PAYER_REVIEW_TABLE_ID")));
