...
public class PreBatchToPayFromFFSInsti extends TestBase {

	MyLogger log = new MyLogger(PreBatchToPayFromFFSInsti.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPayFromFFSInsti");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void commonSteps() throws Exception {
		...
		QMPage.selectClaimFromPrebatch(elementsProp.getProperty("FFSINSTI_PREBATCH_TABLE_ID"));
		ffsInsti.clickOnBtachClaimToPayButton();
	...
	public void movePrebatchToPay() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("FFSINSTI_BATCHTOPAY_TABLE_ID"),2,batchId);
			...
			softAssert.assertEquals(id,
					QMPage.getClaimNumber(elementsProp.getProperty("FFSINSTI_BATCHTOPAY_VIEWCLAIM_TABLE_ID")));
