package com.adjudication.test.viewclaims.capprofessional.batchtopay;
...
public class MoveToEFTPaymentFromCAPPro extends TestBase {

	MyLogger log = new MyLogger(MoveToEFTPaymentFromCAPPro.class.getName());
	...
	public void moveToEFTPaymentFromBatchToPay() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("CAPPRO_EFTPAYMENT_TABLE_ID"),2,batchIdFromApp);
	...
	public void moveToPaidFromEFTPaymentCAPPro() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("CAPPRO_EFTPAYMENT_TABLE_ID"),2,batchId);
			...
			softAssert.assertTrue(viewClaimsPage.checkApproveSuccessMsg(popUpMsg));
			capPro.clickOnPaidTab();
			wait.waitForSeconds(2);
			commonActions.filterData(elementsProp.getProperty("CAPPRO_PAID_TABLE_ID"),1,batchId);
			softAssert.assertEquals(batchId, QMPage.getBatchIdFromPaidWithFilter(elementsProp.getProperty("CAPPRO_PAID_TABLE_ID")));
