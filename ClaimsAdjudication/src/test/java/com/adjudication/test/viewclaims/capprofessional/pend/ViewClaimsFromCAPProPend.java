...
public class ViewClaimsFromCAPProPend extends TestBase {

	MyLogger log = new MyLogger(ViewClaimsFromCAPProPend.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("ViewClaimsFromCAPProPend");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApproveFromCAPProPendNegTC() {
		try {
			log.logInfo(
					"Verify warning messages if claim is moving from pend bucket to Approve bucket with status as pend from CAP pro pend");
			commonSteps();
	...
	public void moveToDenyFromCAPProPendPositiveTC() {
		try {
			log.logInfo(
					"Verify claim is moving from pend to Deny bucket with status as pend from CAP Professional Pend.");
			commonSteps();
	...
	public void checkPRInServiceDetailFromCAPPro() {
		try {
			log.logInfo(
