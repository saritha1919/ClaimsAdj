...
public class PreBatchToPayerReviewFromCAPInsti extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPayerReviewFromCAPInsti.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPayerReviewFromCAPInsti");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			popUpMsg = messageProp.getProperty("PAYERREVIEW_SUCCESS_MESSAGE");
			softAssert.assertTrue(viewClaimsPage.checkApproveSuccessMsg(genericMethods.insertString(popUpMsg, id, 70)));
			wait.waitForSeconds(2);
			capInsti.clickOnOnPayerReview();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("CAPINSTI_PAYER_REVIEW_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("CAPINSTI_PAYER_REVIEW_TABLE_ID")));
