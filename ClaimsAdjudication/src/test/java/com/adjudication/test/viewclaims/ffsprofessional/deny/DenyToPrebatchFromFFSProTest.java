package com.adjudication.test.viewclaims.ffsprofessional.deny;
...
public class DenyToPrebatchFromFFSProTest extends TestBase {

	MyLogger log = new MyLogger(DenyToPrebatchFromFFSProTest.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("DenyToPrebatchFromFFSProTest");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void commonSteps() throws Exception {
		try {
			...
			QMPage.clickOnViewClaims(elementsProp.getProperty("FFSPRO_DENIED_TABLE_ID"));
	...
	public void tryToMoveDenyFromApprovedFromFFSPro() throws Exception {
		try {
			commonSteps();
	...
	public void tryToMovePrebatchFromApprovedFromFFSPro() throws Exception {
		try {
			commonSteps();
			viewClaimsPage.clickOnPrebatchButton();
			...
			softAssert.assertEquals(id,
					QMPage.getClaimNumberFromPrebatch(elementsProp.getProperty("FFSPRO_PREBATCH_TABLE_ID")));
			softAssert.assertAll();
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
