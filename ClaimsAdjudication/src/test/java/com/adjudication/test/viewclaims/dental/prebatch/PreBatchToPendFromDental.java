...
public class PreBatchToPendFromDental extends TestBase{

	MyLogger log = new MyLogger(PreBatchToPendFromDental.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPendFromDental");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void moveToApprovedFromPrebatchPositiveTC() throws Exception {
		try {
			...
			viewClaimsPage.clickOnPendButton();
			QMPage.selectReasonToPend();
			QMPage.clickOnAddInReasonToPend();
			popUpMsg = messageProp.getProperty("PEND_SUCCESS_MESSAGE");
			softAssert.assertTrue(viewClaimsPage.checkApproveSuccessMsg(genericMethods.insertString(popUpMsg, id, 70)));
			wait.waitForSeconds(2);
			dental.clickOnPendTab();
			// clicking on filter and providing the data.
			commonActions.filterData(elementsProp.getProperty("DENTAL_PEND_TABLE_ID"),1,id);
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("DENTAL_PEND_TABLE_ID")));
