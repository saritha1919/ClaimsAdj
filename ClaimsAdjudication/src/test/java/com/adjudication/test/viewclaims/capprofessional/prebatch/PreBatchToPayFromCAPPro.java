...
public class PreBatchToPayFromCAPPro extends TestBase {

	MyLogger log = new MyLogger(PreBatchToPayFromCAPPro.class.getName());
	...
	public void setUp() {
		try {
			...
			id=idProp.getProperty("PreBatchToPayFromCAPPro");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void commonSteps() throws Exception {
		...
		QMPage.selectClaimFromPrebatch(elementsProp.getProperty("CAPPRO_PREBATCH_TABLE_ID"));
		capPro.clickOnBtachClaimToPayButton();
	...
	public void movePrebatchToPay() throws Exception {
		try {
			...
			commonActions.filterData(elementsProp.getProperty("CAPPRO_BATCHTOPAY_TABLE_ID"),2,batchId);
			...
			softAssert.assertEquals(id, QMPage.getClaimNumber(elementsProp.getProperty("CAPPRO_BATCHTOPAY_VIEWCLAIM_TABLE_ID")));
