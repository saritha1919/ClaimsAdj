...
public class FFSProApporvedGridSearchTest extends TestBase {

	MyLogger log = new MyLogger(FFSProApporvedGridSearchTest.class.getName());
	...
	String tr = "//table[@id='FFSPartialAdjudicatedQueTable']/tbody/tr";
	String td = "FFSPartialAdjudicatedQueTable";
	String sheetName ="FFSProApporvedSearchEle";
	...
	@Test
	public void ffsProApporvedGridSearchTest() {
		try {
	...
	public void commonSteps(String data) throws Exception {
		...
		if (commonActions.checkFilteredData(filteredData, data)) {
			...
			//in case if filter data validation is wrong we have fail the test case. that's y we have given true value for assert false.
			softAssert.assertFalse(true);
	...
	public void loadExcelData(int i) {
		...
		resultInTabelXpthWithOutCOB = excel.readData(sheetName,"ResultInTabelWithOutCOB", i);
	...
	public void sortDataBasedonCOB(String getValueToFilter, String data) throws Exception {
		...
		if (data.length() == 0) {
			...
			commonActions.sortData(sortingXapth);
			//After sorting we are checking td count to get data.
			if (QMPage.getSize(getTDCount) == 15) {
