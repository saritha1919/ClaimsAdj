...
public class FFSProPendGridSearchTest extends TestBase {

	MyLogger log = new MyLogger(FFSProPendGridSearchTest.class.getName());
	...
	String tr = "//table[@id='FFSPendQueTable']/tbody/tr";
	String td = "FFSPendQueTable";
	...
	QueueManagementPage ffsProPage;
	Excel excel;
	WaitUtility wait;
	...
	public void setUp() {
		try {
			...
			sheetName = excelProp.getProperty("FFSPRO_PEND_GRIDSEARCH_ELE");
		} catch (Exception e) {
			e.printStackTrace();
	...
	@Test
	public void ffsProPendGridSearchTest() {
		try {
			...
			if (!commonActions.checkNoDataAvail()) {
				for (int i = 1; i < rowCount; i++) {
					loadExcelData(i);
					ffsProPage = new QueueManagementPage(DriverManager.getWebDriver());
					filterData = new LinkedHashMap<String, String>();
					if (ffsProPage.getSize(getTDCount) == 15) {
